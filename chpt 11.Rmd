...
summit_pov <- county %>%
  filter(state == "CO") %>%
  filter(name == "Summit County") %>%
  summarise(avg = mean(poverty)) %>%
  pull(avg)

avg_less_hs <- county %>%
  summarise(avg = mean(less_than_hs)) %>%
  pull(avg)
...
ggplot(county, aes(less_than_hs, poverty)) + geom_jitter() + geom_smooth(method = lm, se = FALSE)
...
#adding animation by region, which is differentiated by color

ggplot(county, aes(less_than_hs, poverty, color = region)) + geom_jitter() + theme_classic() + transition_manual(region) + labs( x= "Percent of Adults with Less than a High School Diploma", y = "Poverty Level", title = "Relationship of Education Levels and Poverty")
...
- model for explanation
  - plotted to find if there was any correlation between the two variables, so it is a model of explanation
...
fundamental problem = You can't have both good education and bad education in a county at the same time, so you can never directly observe the levels of poverty

Fifth, how do the above answers change if, instead of using `good_education`, we use `less_than_hs` instead?

Units = counties
Treatment = having a less than HS education
outcomes = poverty levels
fundamental problem = You can't differing levels of less than a hs education
...
**Prompt** Take a look at the babynames library. Create this animation: https://rpubs.com/ruchajoshi/bennettameter was added to the code chunk to prevent printing of the R code that generated the plot.
